...
Ported from gstack / graphstack pre-landing review. Used by **The Cleaner**. The Safecracker computes the diff (`git diff HEAD` if dirty, else `git diff HEAD~1 HEAD`).

## Instructions

Review the given diff for the issues listed below. Be specific — cite `file:line` and suggest fixes. Skip anything that's fine. Only flag real problems.
...
- **Specialist categories (handled by parallel Cleaner dispatches, NOT this checklist):** Test Gaps, Dead Code, Magic Numbers, Conditional Side Effects, Performance & Bundle Impact, Crypto & Entropy. See this plugin's `specialists/` directory.

The Safecracker applies Fix-First after Cleaner handback: obvious mechanical fixes go in immediately; genuinely ambiguous issues are batched into one user question.
...
- **Check allowlists/filter arrays.** Search for arrays or list literals containing sibling values (e.g., if adding "revise" to tiers, find every array of tier names and verify "revise" is included where needed).
...
#### Dead Code & Consistency (version/changelog only — other items handled by the maintainability specialist)
...
#### Completeness Gaps
- Shortcut implementations where the complete version would cost a modest amount of additional work (e.g., partial enum handling, incomplete error paths, missing edge cases that are straightforward to add)
- Test coverage gaps where adding the missing tests is small (e.g., missing negative-path tests, missing edge case tests that mirror happy-path structure)
- Features implemented at 80-90% when 100% is achievable with modest additional code

#### Time Window Safety
- Date-key lookups that assume "today" covers 24h — a report generated at 8am only sees midnight→8am under today's key
...
- O(n*m) lookups in views (`Array#find` in a loop instead of an indexed lookup)
- Application-side filtering on DB results that could be a `WHERE` clause (unless intentionally avoiding leading-wildcard `LIKE`)

#### Distribution & CI/CD Pipeline
- CI/CD workflow changes (`.github/workflows/` and similar): verify build tool versions match project requirements, artifact names/paths are correct, secrets use the platform's secret-reference syntax, not hardcoded values
...
- Publish step idempotency: re-running the publish workflow should not fail
...
```
CRITICAL (highest severity):      INFORMATIONAL (checklist pass):  SPECIALIST (parallel subagents):
...
All findings are actioned via Fix-First triage. Severity determines
...
This heuristic is used by the Review stage's Fix-First triage to determine whether to auto-fix a finding or ask the user.
